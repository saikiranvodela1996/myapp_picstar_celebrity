...
public class MyImageView extends AppCompatImageView {
    ...
    private MyImageView.TouchArea mTouchArea = MyImageView.TouchArea.OUT_OF_BOUNDS;
    private MyImageView.CropMode mCropMode = MyImageView.CropMode.RATIO_1_1;
    private MyImageView.ShowMode mGuideShowMode = MyImageView.ShowMode.SHOW_ALWAYS;
    private MyImageView.ShowMode mHandleShowMode = MyImageView.ShowMode.SHOW_ALWAYS;
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
        ...
        Drawable drawable;
        mCropMode = MyImageView.CropMode.RATIO_1_1;
        try {
            ...
            for (MyImageView.CropMode mode :MyImageView.CropMode.values()) {
            ...
            for (MyImageView.ShowMode mode : MyImageView.ShowMode.values()) {
    ...
    private void drawEditFrame(Canvas canvas) {
        ...
        if (mCropMode == MyImageView.CropMode.CIRCLE) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                onMove(event);
                if (mTouchArea != MyImageView.TouchArea.OUT_OF_BOUNDS) {
    ...
    private void onUp(MotionEvent e) {
        if (mGuideShowMode == MyImageView.ShowMode.SHOW_ON_TOUCH) mShowGuide = false;
        if (mHandleShowMode == MyImageView.ShowMode.SHOW_ON_TOUCH) mShowHandle = false;
        mTouchArea = MyImageView.TouchArea.OUT_OF_BOUNDS;
        invalidate();
    }
    ...
    private void checkTouchArea(float x, float y) {
        if (isInsideCornerLeftTop(x, y)) {
            mTouchArea = MyImageView.TouchArea.LEFT_TOP;
            if (mHandleShowMode ==MyImageView.ShowMode.SHOW_ON_TOUCH) mShowHandle = true;
            if (mGuideShowMode == MyImageView.ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            return;
        }
        if (isInsideCornerRightTop(x, y)) {
            mTouchArea = MyImageView.TouchArea.RIGHT_TOP;
            if (mHandleShowMode ==MyImageView.ShowMode.SHOW_ON_TOUCH) mShowHandle = true;
            if (mGuideShowMode ==MyImageView.ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            return;
        }
        if (isInsideCornerLeftBottom(x, y)) {
            mTouchArea = MyImageView.TouchArea.LEFT_BOTTOM;
            if (mHandleShowMode == MyImageView.ShowMode.SHOW_ON_TOUCH) mShowHandle = true;
            if (mGuideShowMode == MyImageView.ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            return;
        }
        if (isInsideCornerRightBottom(x, y)) {
            mTouchArea = MyImageView.TouchArea.RIGHT_BOTTOM;
            if (mHandleShowMode ==MyImageView.ShowMode.SHOW_ON_TOUCH) mShowHandle = true;
            if (mGuideShowMode ==MyImageView.ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            return;
        }
        if (isInsideFrame(x, y)) {
            if (mGuideShowMode == MyImageView.ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            mTouchArea =MyImageView.TouchArea.CENTER;
            return;
        }
        mTouchArea = MyImageView.TouchArea.OUT_OF_BOUNDS;
    }
    ...
    private void moveHandleLT(float diffX, float diffY) {
        if (mCropMode == MyImageView.CropMode.RATIO_FREE) {
     ...
     */
    public void rotateImage(MyImageView.RotateDegrees degrees) {
    ...
    public Bitmap getCroppedBitmap() {
        ...
        if (mCropMode !=MyImageView.CropMode.CIRCLE) return cropped;
     ...
     */
    public void setCropMode(MyImageView.CropMode mode) {
        if (mode ==MyImageView.CropMode.RATIO_CUSTOM) {
            setCustomRatio(1, 1);
        } else {
            mCropMode = mode;
            adjustRatio();
    ...
    public void setCustomRatio(int ratioX, int ratioY) {
        ...
        mCropMode =MyImageView.CropMode.RATIO_CUSTOM;
     ...
     */
    public void setGuideShowMode(MyImageView.ShowMode mode) {
        mGuideShowMode = mode;
        switch (mode) {
            case SHOW_ALWAYS:
                mShowGuide = true;
     ...
     */
    public void setHandleShowMode(MyImageView.ShowMode mode) {
        mHandleShowMode = mode;
        switch (mode) {
            case SHOW_ALWAYS:
                mShowHandle = true;
